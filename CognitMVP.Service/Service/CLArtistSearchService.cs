...
namespace DacsOnline.Service.Service
{
    public class CLArtistSearchService : BaseService<ICLArtistSearchManager>, ICLArtistSearchService
    {
        ...
        public CLArtistSearchService(ICLArtistSearchManager employeeServiceManager)
        ...
        /// <returns></returns>
        public List<ArtistCLModel> GetArtists(string Year, string ArtistFirstName, string ArtistLastName, int Pgae, int PageSize, out int TotalItems, string dataSource)
        {
            List<ArtistCLModel> model = null;
            int totalItems;
            try
            {
                List<ArtistCLModel> artists = ServiceManager.SearchArtist(Year, ArtistFirstName, ArtistLastName, Pgae, PageSize, out totalItems, dataSource);
